		...
		if (!context->Written) {
			...
			if (!NT_SUCCESS(status)) {
				...
				if (SendClientPort) {
					...
					auto msg = (FileBackupPortMessage*)ExAllocatePool2(POOL_FLAG_PAGED, len, DRIVER_TAG);
					if (msg) {
...
FLT_POSTOP_CALLBACK_STATUS FileBackupPostCreate(PFLT_CALLBACK_DATA Data, PCFLT_RELATED_OBJECTS FltObjects, PVOID CompletionContext, FLT_POST_OPERATION_FLAGS Flags) {
	...
	context->FileName.Buffer = (WCHAR*)ExAllocatePool2(POOL_FLAG_PAGED, fileNameInfo->Name.Length, DRIVER_TAG);
...
bool IsBackupDirectory(_In_ PCUNICODE_STRING directory) {
	...
	auto copy = (WCHAR*)ExAllocatePool2(POOL_FLAG_PAGED, maxSize + sizeof(WCHAR), DRIVER_TAG);
	if (!copy)
		return false;
